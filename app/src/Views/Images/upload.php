    ...
    <form class="mt-4" action="/images/upload" method="post" enctype="multipart/form-data">
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="name" name="name" placeholder="Enter image name" required value="<?php 
                echo (isset($viewModel) && isset($viewModel->name) ? htmlspecialchars($viewModel->name, ENT_QUOTES, "UTF-8") : "")
            ?>">
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="description" name="description" placeholder="Enter description" required value="<?php 
                echo (isset($viewModel) && isset($viewModel->description) ? htmlspecialchars($viewModel->description, ENT_QUOTES, "UTF-8") : "")
            ?>">
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="alt_text" name="alt_text" rows="2"><?php echo (isset($viewModel) && isset($viewModel->altText) ? htmlspecialchars($viewModel->altText, ENT_QUOTES, "UTF-8") : "")?></textarea>
        </div>
